...
def parse_error(missing, line_num, lexeme, opcode, input_stream):
    print_error(f"ERROR {line_num}: Missing {missing} in {opcode}.")
    # skip to a new line

    # Skip tokens until a NEWLINE or ENDFILE is encountered
    token_category = None
    while token_category != EOL and token_category != EOF:
...
def next_token(input_stream):
    ...
    if token_data is None:
        print_error("ERROR: scan() returned None.")
        return

    if len(token_data) != 3:
        print_error(f"ERROR: scan() returned unexpected data: {token_data}.")
        return
...
def finish_arithop(input_stream):
    ...
    next_token(input_stream)
    if token_category == EOL:
        ir_operation = IR(op_token, curr_lexeme, line_num, arg1, arg2, arg3)
...
def finish_memop(input_stream):
    ...
    if token_category == EOL:
        ir_operation = IR(op_token, curr_lexeme, line_num, arg1, arg2)
        block.insert(ir_operation)
        num_ops += 1
    else:
        #parse_error("newline", op_token)
...
def finish_nop(input_stream):
    ...
    if token_category == EOL:
        ir_operation = IR(op_token, curr_lexeme, line_num)
        block.insert(ir_operation)
        global num_ops
        num_ops += 1
    else:
...
def finish_output(input_stream):
    ...
    if token_category == EOL:
        ir_operation = IR(op_token, curr_lexeme, line_num, arg1)
        block.insert(ir_operation)
        global num_ops
        num_ops += 1
    else:
